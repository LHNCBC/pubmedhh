...
function ABdisplay($id) {
  ...
  global $from, $t, $stopwords, $t_message;
...
if ($from != 'da') {
  ...
  //echo $t_message."---<p>";
  //echo "[<a href=newparser.php?submit=Y&page=$page&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv>Back</a>] ";
	if (substr($from, -3) == 'tbl') {
	  //echo "[<a href=/nlmb/>New search</a>] ";
	}
	elseif (substr($from, -4) == 'tblc') {
	 // echo "[<a href=/nlmc/>New search</a>] ";
	}
	elseif (substr($from, -4) == 'tbld') {
	  if ($outid != '') {
		//  echo "[<a href=/nlmd/index_out.php?outid=$outid>New search</a>] ";
	  }
	  else{
	   // echo "[<a href=/nlmd/>New search</a>] ";
	  }
	}
}
else {
...
if ($from == 'meme') {
  if (strlen($t_message) > 30) {
	$background_path = CLASSES.'/meme/background/';
	$files = array_slice(scandir($background_path), 2);
	$back_index = rand(0, sizeof($files)-1);
	//print_r($files);
	$back_img_file = $files[$back_index];//"gimp_nlm_bldg38A.jpg";
	$jpg_image = imagecreatefromjpeg($background_path.$back_img_file);

      // Allocate A Color For The Text
      $white = imagecolorallocate($jpg_image, 255, 255, 255);
	  $blue = imagecolorallocate($jpg_image, 231, 57, 6);
	  
	  $font_color = $white;
	  if (strpos($back_img_file, "_white.jpg") ) {
			$font_color = $blue;
	  }
	  
      // Set Path to Font File
      $font_path = CLASSES.'/meme/vera.TTF';
	  ...
	  elseif ((strlen($text) > 260 ) && (strlen($text) <450 ) ){
		$font_size = 16;  
      ...
      // Print Text On Image
      imagettftext($jpg_image, $font_size, 0, 55, 40, $font_color, $font_path, $text);
	  
	  $qr_text = urlencode("https://www.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?cmd=prlinks&dbfrom=pubmed&id=".$id."&retmode=ref"); 
	  $url_q = "https://pubmedhh.nlm.nih.gov/nlm/qrenco.php?d=".$qr_text."&t=j";
	  $data = file_get_contents($url_q);
	
	$qr_img = imagecreatefromstring($data);
    ...
    $stamp_image = imagecreatetruecolor($stamp_width, $stamp_height);
    imagecopyresampled($stamp_image, $qr_img, 0, 0, 0, 0, $stamp_width, $stamp_height, $sx, $sy);
	$stamp = $stamp_image; //$qr_img;//
	$sx = imagesx($stamp);
	$sy = imagesy($stamp);
	
	// Copy the stamp image onto our photo using the margin offsets and the photo 
	// width to calculate positioning of the stamp. 
	imagecopy($jpg_image, $stamp, imagesx($jpg_image) - $sx - $marge_right, imagesy($jpg_image) - $sy - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp));
	
	$stamp2 = imagecreatefromjpeg(CLASSES.'/meme/nlm.jpg');
	$sx2 = imagesx($stamp2);
	$sy2 = imagesy($stamp2);
	...
	imagecopy($jpg_image, $stamp2, 5, imagesy($jpg_image) - $sy2 - $marge_bottom2, 0, 0, imagesx($stamp2), imagesy($stamp2));
