...
<?php
  ...
  $logterm = isset($logterm)? urldecode($logterm):'';
	$homeurl = "../pico/consensus.php?outid=$outid&proj=$proj";
	if ($proj == 'abon') {
		$homeurl = "http://pubmedhh.nlm.nih.gov/ebm.php";
	}
?>
<script>
function showAbstract(ab, arrayid, pmid) {
	...
	?>
    if (y == 'block') {
	 url = 'send_pmid.php?'+'type=abstract&from=CONpico&pmid=' + pmid + '&key=' + <?php echo "'".$key00."'"; ?> + '&query=' +<?php echo "'".urlencode($logterm)."'"?>;
...
function showTbl(ab, arrayid, pmid) {
	...
	?>
    if (y == 'block') {
	 url = 'send_pmid.php?'+'type=tbl&from=CONpico&pmid=' + pmid + '&key=' + <?php echo "'".$key00."'"; ?> + '&query=' +<?php echo "'".urlencode($logterm)."'"?>;
...
function searchQue($query) {
  ...
  $esearch = "$utils/esearch.fcgi?" .
                "db=pubmed&retmax=1&usehistory=y&db=pubmed&tool=pmhh&email=pubmedhh@nlm.nih.gov".$query;
...
function display($begin) { 
  ...
  global $Count, $QueryKey, $WebEnv, $page, $homeurl, $from, $logterm, $outid, $proj, $lang, $t, $stopwords;
	global $max_id;
	$max_id = array();
  ...
  print "$Count results: <hr>";
	...
	$from = 'cpico';
	print "<FORM ACTION=consensus_search2.php"."?submit=Y&from=$from&proj=$proj&lang=$lang&outid=$outid&t=$t&logterm=".urlencode($logterm)." METHOD=POST name=form1>";
  ...
  {
		if ($proj == "par") {
		   echo "[<a href=\"http://pubmedhh.nlm.nih.gov/app_abstract_useful.php?id=$pmid&lang=$lang&proj=$proj\" style=\"text-decoration:none\"  target=blank>TBL</a>] ";
		   echo "[<a href=\"http://pubmedhh.nlm.nih.gov/app_abstract_full_useful.php?id=$pmid&lang=$lang&proj=$proj\" style=\"text-decoration:none\"  target=blank>Abstract</a>] ";
		}
		elseif ($proj == "nppar") {
		   echo "[<a href=\"http://pubmedhh.nlm.nih.gov/app_abstract_np.php?id=$pmid&lang=$lang&proj=$proj\" style=\"text-decoration:none\"  target=blank>TBL</a>] ";
		   echo "[<a href=\"http://pubmedhh.nlm.nih.gov/app_abstract_full_np.php?id=$pmid&lang=$lang&proj=$proj\" style=\"text-decoration:none\"  target=blank>Abstract</a>] ";
		}		
		else {	
	...
	if ($outid != '') {
	  ...
	  echo " [<a href=http://www.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?cmd=prlinks&dbfrom=pubmed&id=$pmid&retmode=ref target=new>Full Text</a>]";
	}
...
function related($rid) {
  ...
  $elink = "$utils/elink.fcgi?" .
               "dbfrom=$db&id=$rid&cmd=neighbor&tool=pmhh&email=pubmedhh@nlm.nih.gov";
  ...
  $sbegin = strpos($rest,'<Id>');
	...
	$sbegin = strpos($rest,'<Id>');
  $i = 0;
  global $page, $from, $outid, $proj, $lang, $t, $logterm, $stopwords;
	global $max_id;
	$max_id = array();
  ...
  for($j=$begin; $j<$begin+20; $j++) {
    ...
    echo " [<a href=".$_SERVER['PHP_SELF']."?id=$pmid&mod=related&page=1&outid=$outid&proj=$proj>Related</a>]&nbsp;&nbsp;";
...
if (!(isset($submit) && $submit == "Y")) {
  ...
  $ip = get_client_ip();
  $temp = "PICO CONSENSUS LOG:" .$ip."-(".date("D M j H:i:s Y")."). ";
  ...
  if ($submit == "Y") {
	...
	$page = 1;
     ...
     $beg = ($page-1) * 20;
		 print "US National Library of Medicine<br>";
     call_user_func ('display',$beg);
  }
} //end else -- more pages
...
if (($page > 1) && ($page <= $Tpage)){
  echo "<form action=".$_SERVER['PHP_SELF']."?submit=Y&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&outid=$outid&proj=$proj&t=$t&logterm=".urlencode($logterm)." method=post name=form2>";
	echo "<input type=hidden name=carry value=\"".(array_key_exists('carry', $_POST)? $_POST["carry"]:'')."\">";
...
if (($page == 1) && ($page < $Tpage)) {
	echo "<form action=".$_SERVER['PHP_SELF']."?submit=Y&page=$next&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&outid=$outid&proj=$proj&t=$t&logterm=".urlencode($logterm)." method=post name=form2>";
	echo "<input type=hidden name=carry value=\"".(array_key_exists('carry', $_POST)? $_POST["carry"]:'')."\">";
