  ...
  else
  {
	  if ($hand eq 'tbl') {
		  ...
		  print "[<a href=".$tbl_script."?id=$pmid&from=picotblc>TBL</a>]    ";
			...
			print "[<a href=/abstracts/abstract_tbl.php?id=$pmid&from=picotbld>TBL</a>]    ";
			print "[<a href=/cgi-bin/abstract.cgi?id=$pmid&from=jbtbld&index=$index&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv>Abstract</a>]    ";
		}
		elsif ($hand eq 'tble') {
    ...
    print " [<a href=http://www.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?cmd=prlinks&dbfrom=pubmed&id=$pmid&retmode=ref>Full Text</a>]    ";
  	print " [<a href=/cgi-bin/displayjb.cgi?id=$pmid&mod=related&page=1&hand=$hand&proj=$proj>Related</a>]<br>";
	}
  print "</p>";
...
sub related {
  ...
  { 
		if ($hand eq 'tbl') {
		  ...
		  print "[<a href=".$tbl_script."?id=$pmid&from=picotblc>TBL</a>]    ";
			...
			print "[<a href=/abstracts/abstract_tbl.php?id=$pmid&from=$from>TBL</a>]    ";
  ...
  if ($page ne 1) 
  {
    print "[<a href=/cgi-bin/displayjb.cgi?id=$rid&mod=related&page=$pre&hand=$hand&proj=$proj>Previous</a>] ";
    if ($page ne $Tpage) {
	print "  ";
	print " [<a href=/cgi-bin/displayjb.cgi?id=$rid&mod=related&page=$next&hand=$hand&proj=$proj>Next</a>]";
    }
  }
  ...
  if($page eq 1)
  {
	...
	if($hand eq 'tbl'){
		...
		if ($from eq 'meme') {
			print "        [<a href=/abstracts/index.php?from=$from>New Search</a>]";	
		}
		else {
			print "        [<a href=/abstracts/index.php>New Search</a>]";
		}
	    ...
	    print "        [<a href=/abstracts/index.php>New Search</a>]";
			...
			print "		 		 [<a href=/projects/pubmed/jbrowse.php?&proj=$proj>New Search</a>]";
	}
	else {
    print "        [<a href=/abstracts/index.php>New Search</a>]";
  }
  print "<FORM ACTION=\"./displayjb.cgi?id=$rid&mod=relpager&hand=$hand&proj=$proj\" METHOD=\"POST\">\n";
...
if ($page > 1) {
  print "[<a href=/cgi-bin/displayjb.cgi?submit=Y&page=$pre&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&hand=$hand&proj=$proj>Previous</a>] ";
  if ($page < $Tpage) {
	print "    ";
  	print " [<a href=/cgi-bin/displayjb.cgi?submit=Y&page=$next&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&hand=$hand&proj=$proj>Next</a>]";
  }
}
...
print "<FORM ACTION=\"./displayjb.cgi?submit=Y&mod=pager&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&hand=$hand&proj=$proj\" METHOD=\"POST\">\n";
print "<INPUT type=\"submit\" value=\"page\">\n";
