...
<?php
  ...
  if (is_array($query)) {
    ...
    $homepage = "http://askmedline.nlm.nih.gov/sms";
   	...
   	break;
	case 'spa':
    $homepage = "http://askmedline.nlm.nih.gov/crosslan/index_spa.php";
    ...
    break;
	case 'fre':
    $homepage = "http://askmedline.nlm.nih.gov/crosslan/index_fre.php";
    ...
    break;
	case 'por':
    $homepage = "http://askmedline.nlm.nih.gov/crosslan/index_por.php";
		$message1 = "Retorne à busca";
		$message3 = "Seguida";
    break;
	case 'mix':
    $homepage = "http://askmedline.nlm.nih.gov/crosslan/index_mix.php";
		$message1 = "New Search";
		$message3 = "Next";
    break;
	case 'askq':
    $homepage = "otherq.php";
		$message1 = "New Search";
		$message3 = "Next";
		if ((!isset($term) || ($term == '')) && (isset($pqid) && $pqid != '')) {  
			 $db_object = db_connect('ask1');
			 $pqid = my_decr($pqid);
			 $query = "SELECT question, mesh FROM question WHERE id ='".$pqid."'";
			 $result = mysqli_query($db_object, $query) or die("Query failed : ");// . mysqli_error());
			 $line = mysqli_fetch_array($result);
			 $term = str_replace(',', '',$line["mesh"]);
			 $ques = $line["question"];
		}
  break;
	default:
...
function searchQue($query) {
  ...
  //$utils = "http://www.ncbi.nlm.nih.gov/entrez/eutils";
	...
	$esearch = "$utils/esearch.fcgi?" .
                "db=pubmed&retmax=1&usehistory=y&db=pubmed&api_key=$ncbi_key".$query;
...
function display($begin) { 
  ...
  global $Count, $QueryKey, $WebEnv, $homepage, $message1, $message2, $message3, $from, $outid, $outlibs, $ncbi_key;
  $retstart = 0;
  if (($Count == "")||($Count==0)) {
	  $Count = 0;
		echo "No result!";
  ...
  else
  {
	  print "[<a href=abstract.php?id=$pmid target=new>Abstract</a>]&nbsp;&nbsp;";
		print "[<a href=abstract_tbl.php?id=$pmid target=new>TBL</a>]&nbsp;&nbsp;";
  }
	if ($outid != '') {
	   print " [<a href=".$outlibs[$outid][0]."$pmid target=new>OTool</a>]&nbsp;&nbsp";
  }
	else {
	  print " [<a href=https://www.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?cmd=prlinks&dbfrom=pubmed&id=$pmid&retmode=ref target=new>Full Text</a>]&nbsp;&nbsp;";
	}
  print " [<a href=".$_SERVER['PHP_SELF']."?id=$pmid&mod=related&page=1&from=$from&outid=$outid>Related</a>]&nbsp;&nbsp;";
  print "<br></p>";
  } 
} //end func display
...
function related($rid) {
  global $homepage, $homepage, $message1, $message2, $message3, $from, $outid, $outlibs, $ncbi_key;

	$utils = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils";
  $db     = "pubmed";
  $report = "abstract";

  $elink = "$utils/elink.fcgi?dbfrom=$db&id=$rid&cmd=neighbor&api_key=$ncbi_key";
  ...
  if ($page != 1) 
  {
    print "[<a href=".$_SERVER['PHP_SELF']."?id=$rid&mod=related&page=$pre&from=$from&outid=$outid>Previous</a>] ";
    if ($page != $Tpage) {
	print "&nbsp;";
	print " [<a href=".$_SERVER['PHP_SELF']."?id=$rid&mod=related&page=$next&from=$from&outid=$outid>".$message3."</a>]";
    }
  }
  ...
  print "&nbsp;&nbsp;&nbsp;&nbsp;[<a href=$homepage>".$message1."</a>]";

  print "<FORM ACTION=".$_SERVER['PHP_SELF']."?id=$rid&mod=relpager&from=$from&outid=$outid METHOD=POST>";
...
if ((!isset($id) || ($id != "")) && (isset($mod) && (($mod == "related") || ($mod == "relpager")))) {	
...
if (!(isset($submit) && ($submit=="Y"))) {
  ...
  if($from == 'askq') {
	  $string = "&term=human%5Bmh%5D%20AND%20English%5BLang%5D%20AND%20".urlencode($term);
	}
	else { 
...
if (($page > 1) && ($page <= $Tpage)){
  print "[<a href=".$_SERVER['PHP_SELF']."?submit=Y&page=$pre&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&from=$from&outid=$outid>Previous</a>] ";
  if ($page < $Tpage) {
	print "  &nbsp;";
  	print " [<a href=".$_SERVER['PHP_SELF']."?submit=Y&page=$next&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&from=$from&outid=$outid>".$message3."</a>]";
  }
}
...
echo "<FORM ACTION=".$_SERVER['PHP_SELF']."?submit=Y&mod=pager&Count=$Count&QueryKey=$QueryKey&WebEnv=$WebEnv&from=$from&outid=$outid METHOD=POST>";
print "<INPUT type=\"submit\" value=\"page\">\n";
